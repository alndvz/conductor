...
- [ ] Add `"permission": "allow"` to the conductor, feature, implementor, and review agent configs in `.opencode/opencode.json` so the conductor workflow runs without permission prompts. Scope to these four agents only (not global), so built-in agents retain their defaults.
